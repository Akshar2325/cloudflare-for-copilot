...
declare module 'vscode' {
	...
	export namespace lm {
		...
		*/
		...
		/**
		 * Registers a single chat model with per-model metadata. Introduced in VS Code 1.104.
		 * Call once per model ID; each call registers exactly one model in the Copilot model picker.
		 * @param id Unique model identifier (e.g. "cf-kimi-k2.6")
		 * @param provider The provider that will serve requests for this model
		 * @param metadata Static metadata shown in the model picker
		 * @returns A disposable that unregisters the model when disposed
		 */
		export function registerChatModelProvider(id: string, provider: LanguageModelChatProvider, metadata: ChatModelProviderMetadata): Disposable;
	}

	export interface ChatModelProviderMetadata {
		readonly vendor: string;
		readonly name: string;
		readonly family: string;
		readonly version: string;
